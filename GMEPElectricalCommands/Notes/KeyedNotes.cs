...
namespace ElectricalCommands.Notes
{
  public partial class KeyedNotes : Form
  {
    ...
    public Dictionary<
      string,
      ObservableCollection<ElectricalKeyedNoteTable>
    > KeyedNoteTables { get; set; } =
    ...
    public KeyedNotes()
    {
      ...
      KeyedNoteTables = gmepDb.GetKeyedNoteTables(electricalProjectId);
      InitializeComponent();
    ...
    public void TabControl_Load(object sender, EventArgs e)
    {
      ...
      foreach (var sheetId in KeyedNoteTables.Keys)
      {
        foreach (var table in KeyedNoteTables[sheetId])
        {
          ...
          TabPage newTabPage = new TabPage(title) { BackColor = Color.AliceBlue };
    ...
    {
      TabPage addNewTabPage = new TabPage("ADD NEW") { BackColor = Color.AliceBlue };
    ...
    {
      gmepDb.UpdateKeyNotesTables(projectId, electricalProjectId, KeyedNoteTables);
      UpdateCAD();
    }
    ...
    private void UpdateCAD()
    {
      ...
      )
      {
          ...
          foreach (ObjectId btrId in bt)
          {
            ...
            foreach (ObjectId entId in btr)
            {
              ...
              if (ent is Table)
              {
                ...
                DBDictionary extDict = (DBDictionary)
                  tr.GetObject(table.ExtensionDictionary, OpenMode.ForRead);
                if (extDict != null)
                {
        ...
        {
          BlockTable bt = (BlockTable)tr.GetObject(db.BlockTableId, OpenMode.ForRead);
          BlockTableRecord keyedNoteBlock = (BlockTableRecord)
            tr.GetObject(bt["KEYED NOTE (GENERAL)"], OpenMode.ForRead);
          //Iterating through each block table record
            ...
            if (note.Id == keyedNoteId)
            {
              foreach (ObjectId attId in noteRef.AttributeCollection)
              {
                AttributeReference attRef = (AttributeReference)
                  tr.GetObject(attId, OpenMode.ForWrite);
                if (attRef.Tag == "A")
                {
    ...
    private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ...
      if (tabIndex >= 0 && tabIndex < TableTabControl.TabCount)
      {
        ...
        if (
          tabPage.Controls.Count > 0
          && tabPage.Controls[0] is NoteTableUserControl noteTableUserControl
        )
        {
        ...
        if (ppr.Status == PromptStatus.OK)
        {
          ...
          using (Transaction tr = db.TransactionManager.StartTransaction())
          {
            ...
            for (int i = 0; i < noteTable.KeyedNotes.Count; i++)
            {
              ...
              table
                .Cells[i + 2, 0]
                .SetBlockAttributeValue(attDefId, noteTable.KeyedNotes[i].Index.ToString());
              table.Cells[i + 2, 0].Alignment = CellAlignment.TopCenter;
            ...
            using (
              DBDictionary extDict = (DBDictionary)
                tr.GetObject(table.ExtensionDictionary, OpenMode.ForWrite)
            )
            {
              Xrecord xRec = new Xrecord();

              xRec.Data = new ResultBuffer(new TypedValue((int)DxfCode.Text, noteTable.Id));
              if (!extDict.Contains("gmep_keyed_note_table_id"))
              {
  ...
  public class ElectricalKeyedNoteTable
  {
    ...
    public BindingList<ElectricalKeyedNote> KeyedNotes { get; set; } =
      new BindingList<ElectricalKeyedNote>();
  }
}
