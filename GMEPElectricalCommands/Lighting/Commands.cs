  ...
  public class Commands
  {
    ...
    public static void LightingControlDiagram()
    {
      ...
      using (Transaction tr = db.TransactionManager.StartTransaction())
      {
        ...
        if (!bt.Has("LTG TIMECLOCK")) {
          Autodesk.AutoCAD.ApplicationServices.Application.ShowAlertDialog(
            $"Block 'LTG TIMECLOCK' not found in the BlockTable."
          );
          return;
        }
        if (!bt.Has("LTG LOCATION")) {
          Autodesk.AutoCAD.ApplicationServices.Application.ShowAlertDialog(
            $"Block 'LTG LOCATION' not found in the BlockTable."
          );
          return;
        }
        ...
        List<string> lightingBlockNames = new List<string>
        {
          "GMEP LTG 1X1",
          "GMEP LTG 1X4",
          "GMEP LTG 2X4",
